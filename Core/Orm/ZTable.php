...
class ZTable extends ZOrmAbstract implements \Iterator, \ArrayAccess, \Countable
{
    ...
    */
    public function __construct($tableName, ZDbConnection $db,ZMapper $mapper, $single = false) {
    ...
    */
    public function __destruct() {
        if ($this->connection->cache && !$this->select && isset($this->rows)) {
            ...
            $this->connection->cache->save("$this->table;" . implode(",", $this->conditions), $access);
        }
        $this->rows = null;
        $this->data = null;
    }
    ...
    */
    public function __toString() {
        ...
        if ($this->union) {
            $return = ($this->connection->driver == "sqlite" || $this->connection->driver == "oci" ? $return : "($return)") . implode($this->union);
            if ($this->unionOrder) {
    ...
    */
    public function insert($data) {
        ...
        return new $this->mapper->modelClass($data, $this);
    }
        ...
        if (!isset($this->rows)) {
            ...
            if ($result) {
                ...
                foreach ($result as $key => $row) {
                    ...
                    $className = $this->mapper->modelClass;
